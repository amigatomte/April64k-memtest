...
// -------------------------------
.label COUNTER_BACKUP = $D000 - 1024 - 4 // (SCREEN_BACKUP -4) 4 bytes for backup of the counter. We place this at the end of the $C000 area, just before the 1024 bytes of screen backup storage.
.label BITPATTERN_BACKUP = COUNTER_BACKUP -1 // 1 byte for backup of the bit pattern. We place this just before the counter backup.
// .label COUNTER_BACKUP_FINAL = $FFFE - 4
	// Backup routine: Copy 4 bytes from counter1 ($04) to COUNTER_BACKUP
UnrolledBackup:
	lda counter1                    // load byte from $04
	sta COUNTER_BACKUP              // store byte at COUNTER_BACKUP
	lda counter2                    // load byte from $05
	sta COUNTER_BACKUP+1            // store byte at COUNTER_BACKUP+1
	lda counter3                    // load byte from $06
	sta COUNTER_BACKUP+2            // store byte at COUNTER_BACKUP+2
	lda counter4                    // load byte from $07
	sta COUNTER_BACKUP+3            // store byte at COUNTER_BACKUP+3
        lda.zp rasterirq_main.bitpattern
        sta BITPATTERN_BACKUP
	rts                             // return from subroutine
	
	// Restore routine: Copy 4 bytes from COUNTER_BACKUP back to counter1 ($04)
UnrolledRestore:
	lda COUNTER_BACKUP              // load byte from COUNTER_BACKUP
	sta counter1                    // store byte at $04
	lda COUNTER_BACKUP+1            // load byte from COUNTER_BACKUP+1
	sta counter2                    // store byte at $05
	lda COUNTER_BACKUP+2            // load byte from COUNTER_BACKUP+2
	sta counter3                    // store byte at $06
	lda COUNTER_BACKUP+3            // load byte from COUNTER_BACKUP+3
	sta counter4                    // store byte at $07
        lda BITPATTERN_BACKUP
        sta.zp rasterirq_main.bitpattern
	rts                             // return from subroutine
/*
UnrolledBackup_Final:
	lda counter1                    // load byte from $04
	sta COUNTER_BACKUP_FINAL              // store byte at COUNTER_BACKUP_FINAL
	lda counter2                    // load byte from $05
	sta COUNTER_BACKUP_FINAL+1            // store byte at COUNTER_BACKUP_FINAL+1
	lda counter3                    // load byte from $06
	sta COUNTER_BACKUP_FINAL+2            // store byte at COUNTER_BACKUP_FINAL+2
	lda counter4                    // load byte from $07
	sta COUNTER_BACKUP_FINAL+3            // store byte at COUNTER_BACKUP_FINAL+3
	rts                             // return from subroutine
	
	// Restore routine: Copy 4 bytes from COUNTER_BACKUP_FINAL back to counter1 ($04)
UnrolledRestore_Final:
	lda COUNTER_BACKUP_FINAL              // load byte from COUNTER_BACKUP_FINAL
	sta counter1                    // store byte at $04
	lda COUNTER_BACKUP_FINAL+1            // load byte from COUNTER_BACKUP_FINAL+1
	sta counter2                    // store byte at $05
	lda COUNTER_BACKUP_FINAL+2            // load byte from COUNTER_BACKUP_FINAL+2
	sta counter3                    // store byte at $06
	lda COUNTER_BACKUP_FINAL+3            // load byte from COUNTER_BACKUP_FINAL+3
	sta counter4                    // store byte at $07
	rts                             // return from subroutine
*/
PRINT_COUNTER:                                  // Print the counter value at position (0,20)
        jsr DEC_COUNTER
        lda #DEC_OUT
        sta.zp printstring_main.STR_PTR_LO
        lda #$00
        sta.zp printstring_main.STR_PTR_HI
        lda #8
        sta.zp printstring_main.XPOS
        lda #20
        sta.zp printstring_main.YPOS
        lda #05
        sta.zp printstring_main.CHARCOLOUR
        jsr printstring_main.PRINT_STRING
        rts